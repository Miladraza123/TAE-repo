-- ==========================================================
-- Trial Balance (§19) and Receivable Aging (§20) — live queries/functions,
-- never a separately-posted ledger (§11's structural note: edits/deletes
-- ripple through automatically because these always read source documents).
-- ==========================================================
  ...
  with net as (
    select p.id as party_id, p.name as party_name,
      round(
        ...
        - coalesce((select sum(coalesce(nullif(r ->> 2, '')::numeric, 0)) from sheets s, jsonb_array_elements(s.rows) r
                    where s.deleted_at is null and s.sheet_date <= p_as_of and nullif(r ->> 6, '') is not null and (r ->> 6)::uuid = p.id), 0)
        + coalesce((select sum(coalesce(nullif(r ->> 0, '')::numeric, 0)) from sheets s, jsonb_array_elements(s.rows) r
                    where s.deleted_at is null and s.sheet_date <= p_as_of and nullif(r ->> 7, '') is not null and (r ->> 7)::uuid = p.id), 0)
      , 2) as net_bal
    from parties p
...
begin
  ...
  for p in select * from parties where kind in ('customer','supplier','both') and deleted_at is null loop
    ...
    select v_pool + coalesce(sum(coalesce(nullif(r ->> 2, '')::numeric, 0)), 0) into v_pool
      from sheets s, jsonb_array_elements(s.rows) r
      where s.deleted_at is null and s.sheet_date <= p_as_of
        and nullif(r ->> 6, '') is not null and (r ->> 6)::uuid = p.id;
    ...
    for c in (
      select * from (
        ...
        select 1, (s.sheet_date), (s.sheet_date), 'cash_paid', 'Cash paid'::text, null::uuid,
               coalesce(nullif(r ->> 0, '')::numeric, 0), 0::numeric
        from sheets s, jsonb_array_elements(s.rows) r
        where s.deleted_at is null and s.sheet_date <= p_as_of
          and nullif(r ->> 7, '') is not null and (r ->> 7)::uuid = p.id
      ) charges
...
-- ----------------------------------------------------------
-- aging_reconcile() — standing self-check: aging's net per party must
-- match trial_balance's net per party (§20).
-- ----------------------------------------------------------
create or replace function aging_reconcile(p_as_of date default current_date)
returns table(party_id uuid, party_name text, trial_balance_net numeric, aging_net numeric, difference numeric)
language sql
stable
security definer
set search_path = public
as $$
  with tb as (
    select party_id, party_name, case when side = 'dr' then balance else -balance end as net
    from trial_balance(p_as_of)
  ),
  ag as (
    select party_id, party_name,
      sum(case when bucket in ('payable','advance') then -outstanding else outstanding end) as net
    from receivable_aging(p_as_of)
    group by party_id, party_name
  )
  select coalesce(tb.party_id, ag.party_id), coalesce(tb.party_name, ag.party_name),
         coalesce(tb.net, 0), coalesce(ag.net, 0), round(coalesce(tb.net, 0) - coalesce(ag.net, 0), 2)
  from tb full outer join ag on ag.party_id = tb.party_id
  where round(coalesce(tb.net, 0) - coalesce(ag.net, 0), 2) <> 0;
$$;
...
revoke execute on function trial_balance(date) from anon;
revoke execute on function receivable_aging(date) from anon;
revoke execute on function aging_reconcile(date) from anon;
