-- ==========================================================
-- Fix a regression introduced by 0026: rewriting smart_merge_lines() there
-- to also return 'merged_lines' accidentally reverted its allow-list check
-- from `not (p_line_table = any(...))` back to the buggy `p_line_table <>
-- any(...)` pattern that 0012 had already fixed once before (that operator
-- means "not equal to at least one array element", true for almost any
-- table regardless of whether it's actually in the allow-list — so this
-- raised "table % is not allowed" for every line table, not just invalid
-- ones, breaking the merge-save path for every existing multi-line
-- document, not only Daily Ledger).
--
-- Pure revert of that one check back to 0012's correct form — no other
-- change to the function (0026's 'merged_lines' addition is preserved).
-- ==========================================================

create or replace function smart_merge_lines(
  p_line_table text, p_fk_col text, p_fk_id uuid,
  p_original_lines jsonb, p_new_lines jsonb,
